...
class PolySynth(deviceId:String = "") extends MyReceiver(deviceId) with EnvCallbackDestination with StandardKnobs {
    ...
    msg match {
      case a: ShortMessage => {
        ...
        def newVoiceOld(note: Int):Tuple2[SiGen, List[Env]] = {
          ...
          val env = new Env(attack.getValue(0), 0.5f, 1f, release.getValue(0), Some(this), note)
          val filterEnv = new Env(attack.getValue(0), release.getValue(0), 0.2f, release.getValue(0), None, note)
  ...
  def newVoice(note: Int, detune: Float=0):Tuple2[SiGen, List[Env]] = {
    ...
    val env = new Env(attack.getValue(0), 0.5f, 1f, release.getValue(0), Some(this), note)
    val filterEnv = new Env(attack.getValue(0), release.getValue(0), 0.3f, release.getValue(0), None, note)
...
class BinauralSynth(deviceId:String = "") extends PolySynth(deviceId) {
